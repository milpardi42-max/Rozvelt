...
bash scripts/marketplace-smoke/artist-dashboard-e2e.sh   # the shared signup form + artist dashboard
...
- `artist-dashboard-e2e.sh` is the registration rig: it asserts buyers and designers share **one**
  registration form — `/signup` serves that form, the account type is a real radio choice inside it, the
  seller half is in the same form but closed until it is chosen, the choice (and the markup it toggles)
  ships in the page's own bundle — that `/signup/buyer` serves the very same form under a buyer heading
  with the switch, that `/signup/artist` keeps its sell-side content and opens the same form on the
  seller half (account fields required, the studio block — studio, experience, bio, formats, families,
  terms — optional and no step wizard), that the old `/creators/join` path 307s to the artist page, that
  `POST /api/auth/signup` really stores that file on the Artist record when it is filled in and invents
  nothing when it is not (and that the same e-mail cannot register twice), that the admin sees it, and
  that `/artist` is the dashboard (signed-out redirect, buyer upsell, artist KPIs/works/delivery/wallet)
  while `/artist/portfolio` still serves the portfolio manager. It creates and deletes its own accounts. **Note:** the signup endpoint
